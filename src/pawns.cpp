...
#undef V
    ...
    Score evaluate(const Position &pos, Pawns::Entry *e)
    {
      ...
      while (b)
      {
        ...
        passed = !(stoppers ^ lever) || (!(stoppers ^ leverPush) && popcount(phalanx) >= popcount(leverPush)) || (stoppers == blocked && r >= RANK_5 && (shift<Up>(support) & ~(theirPawns | doubleAttackThem)));
        ...
        {
          if (opposed && (ourPawns & forward_file_bb(Them, s)) && !(theirPawns & adjacent_files_bb(s)))
            score -= Doubled;
          else
